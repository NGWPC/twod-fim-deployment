...
CREATE TABLE IF NOT EXISTS materialized_nd_runs(
    ...
    -- The library, ascending: the discharges this reach is ADOPTED to have, and
    -- the ones the KWSE step builds a stage library at. Storage may hold more
    -- runs left behind by an earlier, the loop does not care.
    -- It adopts the smallest set meeting the authored ld_q_* resolution, since
    -- every extra discharge here is a whole extra KWSE stage grid, and warns
    -- about what it passed over so the surplus can be cleaned up.
