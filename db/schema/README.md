...
- `model_id` — `"<identity_hash>_<domain_code>"`, the model folder name in
  storage; in `current_state` it is a **generated** column so it can never drift
  from its parts.
- Tables split by who owns them and whether they can be rebuilt:
  `desired_state` is authored intent, `current_state` + `runs` are a snapshot of
  what storage holds (rebuildable by rescanning S3), and `reach_processing` +
  `reach_activity` are the reconciler's own notes (not rebuildable).
